...
export function BookTable({ bookings, onBooked }) {
  ...
  async function submit() {
    ...
    if (fresh?.some(b => overlaps(form.start_time, form.end_time, b.start_time, b.end_time))) {
      setError('ขออภัย โต๊ะนี้ถูกจองแล้ว กรุณาเลือกโต๊ะอื่น Sorry, table just taken.')
    ...
    <div className="p-4 max-w-lg mx-auto">
      <div className="bg-green-500 text-white rounded-2xl p-6 text-center mb-4">
        ...
        <div className="text-green-100 text-sm space-y-1">
          ...
          <div>ค่าเช่า Fee: <strong>{fmt(confirmed.amount)} THB</strong></div>
        </div>
        <div className="bg-green-400 bg-opacity-50 rounded-xl p-3 text-sm mt-4">
          ชำระเงินสดที่เคาน์เตอร์เมื่อมาถึง Pay cash at counter when you arrive
        </div>
      </div>
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      ...
      <div className="bg-green-50 border border-green-100 rounded-xl px-4 py-3 mb-4 text-xs text-green-700">
        <div className="font-medium">20 ฿ / ชั่วโมง / โต๊ะ (อย่างต่ำ 1 ชั่วโมง) · นักเรียนสูงสุด 4 คน · ชำระเงินสดที่เคาน์เตอร์</div>
        <div className="text-green-500 mt-0.5">20฿/hr/table (Min 1 hr) · Max 4 students · Pay cash at counter</div>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input value={form.booker_name} onChange={e => set('booker_name', e.target.value)} placeholder="ชื่อ-นามสกุล Full name"
        ...
        {timeOk && (
          <div className="text-xs text-green-600 font-medium">
            ระยะเวลา: {Math.floor(hours)} ชั่วโมง{hours % 1 ? ' 30 นาที' : ''} · ค่าเช่า: {fee} ฿
            {' '}| Duration: {Math.floor(hours)} hr{hours % 1 ? ' 30min' : ''} · Fee: {fee} THB
            {loadingAvail && <span className="text-amber-500 ml-2">กำลังตรวจสอบ...</span>}
          </div>
        )}
        <div>
        ...
        <button onClick={submit} disabled={saving || !selectedTable || !timeOk}
          ...
          {saving ? 'กำลังจอง... Booking...' : 'จองโต๊ะ · Confirm' + (fee ? ' · ' + fee + ' THB' : '')}
        </button>
      </div>
...
export function MyBookings({ bookings }) {
  return (
    <div className="p-4">
      ...
      {bookings.length === 0
        ? <div className="bg-white border border-gray-100 rounded-2xl p-8 text-center text-sm text-gray-400">ยังไม่มีการจอง · No bookings yet.</div>
